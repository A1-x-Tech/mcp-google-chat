# Google Chat: Connection status — MCP tool

**Google Chat MCP tool:** Reports whether Google Chat is connected, where the token comes from, when it expires and which account it belongs to.

Technical name: `auth_status`

## What task it solves

> I want to check whether this server is connected to Google Chat and as whom.

Answers the "why does every tool fail" question before anyone starts editing configuration files: token present or not, its source (the `GOOGLE_CHAT_*` environment variables or a stored in-chat login), its expiry, the account email and the paths of the stored client and credentials.
...
Nothing. The tool reads local connection state and does not touch Google Chat.

## Example request

> Show the Google Chat connection status — are we connected, and as which account?
