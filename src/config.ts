...
/** Default Google Chat API host. */
export const DEFAULT_BASE = "https://chat.googleapis.com";
...
export const MISSING_CREDENTIALS_MESSAGE =
  ...
  "GOOGLE_CHAT_ACCESS_TOKEN with a short-lived access token. " +
...
export function hasCredentials(config: GoogleChatConfig): boolean {
 ...
 *
 *   GOOGLE_CHAT_CLIENT_ID      OAuth2 client id      (refresh flow, recommended)
 *   GOOGLE_CHAT_CLIENT_SECRET  OAuth2 client secret  (refresh flow)
 *   GOOGLE_CHAT_REFRESH_TOKEN  OAuth2 refresh token  (refresh flow)
 *   GOOGLE_CHAT_ACCESS_TOKEN   static access token (alternative; expires in ~1h)
 *   GOOGLE_CHAT_API_BASE       API root override (default https://chat.googleapis.com)
 *   GOOGLE_CHAT_TIMEOUT_MS     per-request timeout (default 60000)
 *   GOOGLE_CHAT_MAX_RETRIES    retries on transient errors (default 3)
 */
export function loadConfig(): GoogleChatConfig {
  ...
  const accessToken = process.env.GOOGLE_CHAT_ACCESS_TOKEN;

  const oauthProvided = [clientId, clientSecret, refreshToken].filter(Boolean).length;
  if (oauthProvided > 0 && oauthProvided < 3) {
    throw new ConfigError(
      "GOOGLE_CHAT_CLIENT_ID, GOOGLE_CHAT_CLIENT_SECRET and GOOGLE_CHAT_REFRESH_TOKEN must be set together (OAuth2 refresh flow).",
      "incomplete_oauth_config",
    );
  }

  const timeoutMs = Number(process.env.GOOGLE_CHAT_TIMEOUT_MS);
  const maxRetries = Number(process.env.GOOGLE_CHAT_MAX_RETRIES);
  ...
  return {
    ...
    refreshToken,
    accessToken,
    apiBase: process.env.GOOGLE_CHAT_API_BASE || DEFAULT_BASE,
